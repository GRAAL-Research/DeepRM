...
def train_meta_predictor(config: dict) -> None:
    ...
    train_idx, valid_idx, test_idx = train_valid_and_test_indices(config["dataset"], datasets, config["splits"],
                                                                  ...
                                                                  config["seed"])
    train_loader = create_data_loader(config["dataset"], "train", datasets, config["meta_batch_size"], train_idx)
    valid_loader = create_data_loader(config["dataset"], "valid", datasets, config["meta_batch_size"], valid_idx)
    test_loader = create_data_loader(config["dataset"], "test", datasets, config["meta_batch_size"], test_idx)
