...
def is_stopping_training(config: dict, train_metrics: dict[str, np.ndarray], epoch_idx: int, best_epoch: int,
    ...
    # The minimum performance to achieve depends on the label size.
    min_perf = 0.525 if target_size == 1 else 1 / target_size + 0.025
    has_no_learning_being_made = train_metrics[TRAIN_ACCURACY_MEAN] < min_perf and epoch_idx > 50
