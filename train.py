...
class SimpleNet(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim):
        """
        Generates a simple feed-forward neural network with ReLU activations.
        Args:
...
def train(net, dataset, m, optimizer, scheduler, n_epoch, batch_size, criterion, DEVICE):
    """
    ...
    for i in range(n_epoch):
        ...
        with torch.enable_grad():
            for inputs, targets in train_loader:
                ...
                output = net(torch.reshape(inputs, (batch_size,-1)))
                loss = criterion(output, inputs, targets)
                loss.backward()
                optimizer.step()
        train_acc = perf(net, train_loader, batch_size, DEVICE)
        val_acc = perf(net, valid_loader, batch_size, DEVICE)
...
def BCELoss_mod(output, inputs, targets):
    ...
    """
    out = nn.Sigmoid()(torch.sum(torch.transpose(inputs, 0, 1) * output[:, :-1], dim=-1) + output[:,-1])
    return nn.BCELoss()(torch.transpose(out,0,1), targets)

def perf(net, data_loader, batch_size, DEVICE):
    """
    ...
    with torch.no_grad():
        ...
        for inputs, targets in data_loader:
            ...
            if str(DEVICE) == 'cuda':
                inputs, targets, net = inputs.cuda(), targets.cuda(), net.cuda()
            output = net(torch.reshape(inputs, (batch_size, -1)))
            out = torch.transpose(torch.sign(torch.sum(torch.transpose(inputs, 0, 1) * output[:, :-1], dim=-1) + output[:,-1]), 0, 1)
