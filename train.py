...
class SimpleMetaNet(nn.Module):
    def __init__(self, input_dim, dims, output_dim, m, d, k, tau, batch_size, init):
        ...
        self.modl_2 = relu_fc_layer([self.output_dim_1 - self.m + self.kern_2_dim[-1]] + self.modl_2_dim, last=True)
        self.last_l_1 = relu_fc_layer([self.modl_1_dim[-1]] + [self.output_dim_1])
        self.last_l_2 = relu_fc_layer([self.modl_2_dim[-1]] + [self.output_dim_2])
...
def train(meta_pred, pred, dataset, test_split, valid_split, optimizer, scheduler, tol, early_stop,
          n_epoch, batch_size, criterion, penalty_msg, penalty_msg_coef, vis, bound_type, DEVICE, independent_food):
    """
    ...
    """
    train_loader, valid_loader, test_loader = train_valid_loaders(dataset, batch_size, test_split, valid_split)
    ...
    for i in range(n_epoch):
        ...
        val_acc, val_loss, _ = stats(meta_pred, pred, criterion, valid_loader, n_sigma, None, DEVICE)
        test_acc, test_loss, _ = stats(meta_pred, pred, criterion, test_loader, n_sigma, None, DEVICE)
        ...
        if i - j > early_stop:  # If no improvement for early_stop epoch, stop training.
            break
    if vis > 0 and pred.pred_type == 'linear_classif' and len(inputs[0,0,:-1]) == 2:
...
def stats(meta_pred, pred, criterion, data_loader, n_sigma, bound_type, DEVICE):
    ...
    with torch.no_grad():
        ...
        for inputs, targets in data_loader:
            ...
            pred.update_weights(meta_output)
            output = pred.forward(inputs, True)
            tot_loss += criterion(output[0], targets) * n
            tot_acc += lin_loss(output[1], targets*2-1) * n
            if i == n:
        ...
        if bound_type is not None:
            bnd = compute_bound(bound_type=bound_type, n_Z=meta_pred.k, n_sigma=n_sigma,
                                m=i, r=i - tot_acc, c_2=10, d=len(meta_output), data=np.array(out*tar + 1) / 2, delta=0.05)
